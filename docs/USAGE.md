...
1. **PM2 のインストール**  
   ...
   ```
2. **本番ビルドの準備**  
   `.env` を更新した後、依存を反映してからビルドします。
   ```bash
   npm ci --omit=dev
   npm run build
   ```
3. **PM2 で API サーバーを起動**  
   `.env` の内容は `--update-env` で反映されます。`--time` を付けるとログにタイムスタンプが残ります。
   ```bash
   pm2 start dist/index.js --name vivliostyle-cli-server --update-env --time
   ```
   プロキシサーバーも常駐させたい場合は `dist/proxyServer.js` を別名で登録してください。
   ```bash
   pm2 start dist/proxyServer.js --name vivliostyle-proxy --update-env --time
   ```

### ecosystem.config.js を使った多プロセス管理

`pm2 start ecosystem.config.js` でまとめて起動できます。リポジトリ直下に次のファイルを配置し、`cwd` や `env` を実環境に合わせて調整してください。
  ...
  apps: [
    {
      ...
      cwd: '/opt/GROWI-VivliostyleCLI-Server',
      env: {
        NODE_ENV: 'production',
        PORT: 4781,
        VIV_WORKSPACE_DIR: './jobs'
      }
    },
    {
...
- `pm2 save` で現在のプロセス一覧を保存します。
- `pm2 startup` を実行すると OS の起動時に PM2 が復元されるように systemd / launchd / Windows サービスを登録します（指示されたコマンドを管理者権限で実行してください）。
- 更新時は `git pull` → `npm ci --omit=dev` → `npm run build` → `pm2 reload vivliostyle-cli-server`（必要なら `pm2 reload vivliostyle-proxy`）の順で反映します。
- 状態確認: `pm2 status`
- ログ監視: `pm2 logs vivliostyle-cli-server`
- 停止/削除: `pm2 stop vivliostyle-cli-server`、`pm2 delete vivliostyle-cli-server`

...
const numberFromEnvAllowZero = (value: string | undefined, fallback: number): number => {
  if (value === undefined) {
    return fallback;
  }
  const parsed = Number(value);
...
export const config = {
  ...
  autoCleanupTimeoutMs: numberFromEnvAllowZero(
    process.env.VIV_AUTOCLEANUP_TIMEOUT_MS,
    DEFAULT_AUTOCLEANUP_TIMEOUT_MS,
  ),
  frontendPingUrl: process.env.VIV_FRONTEND_PING_URL ?? "",
  frontendPingTimeoutMs: numberFromEnvAllowZero(
